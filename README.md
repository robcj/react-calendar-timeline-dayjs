# React Calendar Timeline (dayjs)

A modern and responsive React timeline component.

Forked from react-calendar-timeline to use dayjs instead of moment.js
...
```jsx
import Timeline from 'react-calendar-timeline-dayjs'
// make sure you include the timeline stylesheet or the timeline will not be styled
import 'react-calendar-timeline-dayjs/lib/Timeline.css'
...
const items = [
  {
    ...
    title: 'item 1',
    start_time: dayjs(),
    end_time: dayjs().add({hours: 1})
  },
  {
    id: 2,
    group: 2,
    title: 'item 2',
    start_time: dayjs().add({hours: -0.5}),
    end_time: dayjs().add{hours: 0.5})
  },
  {
    id: 3,
    group: 1,
    title: 'item 3',
    start_time: dayjs().add({hours: 2}),
    end_time: dayjs().add({hours: 3})
  }
]
...
ReactDOM.render(
  <div>
    ...
    <Timeline
      groups={groups}
      items={items}
      defaultTimeStart={dayjs().add({hours: -12})}
      defaultTimeEnd={dayjs().add({hours: 12})}
    />
  </div>,
...
const minTime = dayjs().add({months: -6}).valueOf()
const maxTime = dayjs().add({months: 6}).valueOf()
