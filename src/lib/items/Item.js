...
export default class Item extends Component {
  ...
  getItemProps = (props = {}) => {
    ...
    const classNames =
      'rct-item' + (this.props.item.className ? ` ${this.props.item.className}` : '');
  ...
  getItemStyle(props) {
    ...
    let finalStyle = combinedStyle;
    // remove border from final style if we have borderColour, borderWidth and borderStyle
    // to prevent browser warnings about mixing css shorthand properties.
    if (
      finalStyle.border &&
      finalStyle.borderColor &&
      finalStyle.borderWidth &&
      finalStyle.borderStyle
    ) {
      delete finalStyle.border;
    }

    // If we have specific border width properties then remove borderWidth and replace with specific ones.
    if (finalStyle.borderWidth) {
      let deleteBorderWidth = false;
      ['Left', 'Right', 'Top', 'Bottom'].forEach(pos => {
        const width = finalStyle[`border${pos}Width`] || finalStyle.borderWidth;
        finalStyle[`border${pos}Width`] = width;
        deleteBorderWidth = true;
      });
      if (deleteBorderWidth) {
        delete finalStyle.borderWidth;
      }
    }
    return finalStyle;
  }
